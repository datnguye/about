...
def zero_shot_approach(user_query: str) -> str:
    ...
    response = completion(
        ...
        messages=[
            {
                "role": "system",
                "content": "Extract SQL queries from natural language. Return only valid SQL.",
            },
...
def few_shot_approach(user_query: str) -> str:
    ...
    response = completion(
        ...
        messages=[
            ...
            {
                "role": "assistant",
                "content": "SELECT * FROM users WHERE YEAR(created_at) = YEAR(CURDATE());",
            },
            # Actual query
            {"role": "user", "content": user_query},
        ],
...
def advanced_few_shot_with_context(user_query: str, table_schema: dict) -> str:
    ...
    response = completion(
        ...
        messages=[
            ...
            {
                "role": "assistant",
                "content": "SELECT customer_id, name, total_spent FROM customers WHERE total_spent > 10000 ORDER BY total_spent DESC;",
            },
            {"role": "user", "content": "Find recent orders that are still processing"},
            {
                "role": "assistant",
                "content": "SELECT order_id, customer_id, created_at FROM orders WHERE status = 'processing' AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY);",
            },
            # Actual query
            {"role": "user", "content": user_query},
        ],
...
def demonstrate_pattern_learning():
    ...
    patterns = {
        "Error Analysis": {
            "examples": [
                (
                    "This query is slow",
                    "ISSUE: Performance | CAUSE: Missing index | FIX: ADD INDEX idx_user_email ON users(email)",
                ),
                (
                    "Query returns wrong data",
                    "ISSUE: Logic error | CAUSE: Incorrect JOIN condition | FIX: Change INNER JOIN to LEFT JOIN",
                ),
            ],
        ...
        "Code Review": {
            "examples": [
                (
                    "SELECT * FROM users",
                    "RATING: 2/5 | ISSUES: Using SELECT *, no WHERE clause | IMPROVE: Specify columns, add filtering",
                ),
                (
                    "SELECT u.id, u.name FROM users u WHERE u.active = 1",
                    "RATING: 4/5 | ISSUES: None major | IMPROVE: Consider adding LIMIT for large datasets",
                ),
            ],
    ...
    for pattern_name, pattern_data in patterns.items():
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a SQL expert. Follow the exact format shown in examples.",
            }
        ]
